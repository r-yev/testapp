...
function handleRequest(): void
{
    ...
    $searchable = $method . ' ' . $uri;

    if (!in_array($searchable, array_keys(_ROUTES)))
        notFound();

    $route = _ROUTES[$searchable];
    $class = $route[0];
    $method = $route[1];
    $instance = _BEANS[$class];
    ...
    try {
        ...
        {
            if ($requiredParam->allowsNull()) continue;
            if (!key_exists($requiredParam->getName(), $params))
                badRequest();
        }

        try {
            die($reflectionMethod->invoke($instance, ...$params));
...
#[NoReturn] function badRequest(): void
{
